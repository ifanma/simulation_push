...
U = 8 * diag([1, 1, 1, 1]);         % 接触边切换代价，认为不切换最好
...
for k = 1:N
    ...
    edge1 = [x{k}(4)== -1, -1<=x{k}(5)<=1, x{k+1}- x_star{k+1} == S1{k}(:,1:5)*(x{k}-x_star{k}) + S1{k}(:,6:8)*(u{k}-u_star{k})];
    edge2 = [x{k}(5)== 1,  -1<=x{k}(4)<=1, x{k+1}- x_star{k+1} == S2{k}(:,1:5)*(x{k}-x_star{k}) + S2{k}(:,6:8)*(u{k}-u_star{k})];
    edge3 = [x{k}(4)== 1,  -1<=x{k}(5)<=1, x{k+1}- x_star{k+1} == S3{k}(:,1:5)*(x{k}-x_star{k}) + S3{k}(:,6:8)*(u{k}-u_star{k})];
    edge4 = [x{k}(5)== -1, -1<=x{k}(4)<=1, x{k+1}- x_star{k+1} == S4{k}(:,1:5)*(x{k}-x_star{k}) + S4{k}(:,6:8)*(u{k}-u_star{k})];
...
subplot(2, 1, 1)
[~, ind] = max(z_rec);
imagesc(flipud(ind)); axis tight; grid off; colorbar
title('接触状态')
