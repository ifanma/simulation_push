...
param.Q = 2 * diag([20, 20, 10, 0, 5]);    % 状态跟踪代价
param.R = 1 * diag([100, 10, 10]);       % 控制跟踪代价
param.W = 0.2 * diag([0.5, 1, 1]);        % 状态先验代价，认为不滑动的状态较好
param.V = 0.2 * diag([1, 1, 1]);          % 状态切换代价，认为不切换最好
...
SolverLimitTime = 0.1;          % 优化器时间限制
PredictHorizon = 0.3;           % Mpc预测时域
PredictPeriod = 0.02;           % 优化间隔周期
...
createTask(job,@c2_task2, 1, {x0, param});
