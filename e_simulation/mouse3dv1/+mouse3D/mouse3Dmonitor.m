...
%             p = 0.7*obj.R*[src.Sen.Translation.X
%                  src.Sen.Translation.Y
%                  src.Sen.Translation.Z]/1000;
%             r = [src.Sen.Rotation.X;
%                  src.Sen.Rotation.Y;
%                  src.Sen.Rotation.Z];
%             angle = 0.5*src.Sen.Rotation.Angle/180*pi
%             R_curr = [r(1)^2*(1-cos(angle))+cos(angle),r(1)*r(2)*(1-cos(angle))-r(3)*sin(angle),r(1)*r(3)*(1-cos(angle))+r(2)*sin(angle);
%                  r(1)*r(2)*(1-cos(angle))+r(3)*sin(angle),r(2)^2*(1-cos(angle))+cos(angle),r(2)*r(3)*(1-cos(angle))-r(1)*sin(angle);
%                  r(1)*r(3)*(1-cos(angle))-r(2)*sin(angle),r(2)*r(3)*(1-cos(angle))+r(1)*sin(angle),r(3)^2*(1-cos(angle))+cos(angle)];
%             R_des = obj.R*R_curr*obj.R';
%             obj.quadro.setparam(obj.id,p);
%             obj.quadro.setparam(obj.id_Rot,R_des);
            ...
            %norm( [src.Sen.Rotation.X src.Sen.Rotation.Y src.Sen.Rotation.Z] )
        end
